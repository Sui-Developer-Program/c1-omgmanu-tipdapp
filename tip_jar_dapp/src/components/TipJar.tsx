    ...
    const fetchTipJarStats = async () => {
      ...
      try {
        ...
        if (tipJarObject.data?.content && 'fields' in tipJarObject.data.content) {
          const fields = tipJarObject.data.content.fields as {balance: string, owner: string, total_tips: string};
          setTipJarStats({
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="space-y-4">
        ...
        <button
          onClick={sendTip}
          disabled={isLoading || !tipAmount || parseFloat(tipAmount) <= 0 || tipAmount > balance}
          className="w-full bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
        >
          {isLoading ? 'Sending Tip (Gas-Free)...' : tipAmount > balance ? 'Tip amount exceeds balance' : 'Send Tip (Gas-Free)'}
        </button>
      </div>
